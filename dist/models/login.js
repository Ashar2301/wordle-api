...
loginDB.checkIfEmailExists = async (userEmail) => {
    let user = await loginDB.findUserByEmail(userEmail);
    if (user)
        return { code: 200, response: "User found!" };
    return { code: 404, response: "User not found!" };
};
loginDB.sendResetPasswordEmail = async (userEmail) => {
    ...
    let newEmail = email
        ...
        .replace("{{resetPasswordLink}}", `https://wordle-by-ashar.vercel.app/forgotPassword/${token}`);
...
loginDB.resetPassword = async (userEmail, password) => {
    let model = await collection.getUserCollection();
    const hashedPassword = await bcrypt.hash(password, 10);
    let res = await model.findOneAndUpdate({ email: userEmail }, { password: hashedPassword });
    if (res) {
